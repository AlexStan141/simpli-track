<form action="{{ route('note.update', ['bill_id' => $bill_id]) }}" method="POST"
    class="flex flex-col gap-2 mt-5 ml-2 items-start">
    @csrf
    @method('PUT')
    <label for="message">Edit the note</label>
    <textarea name="message" id="message" class="w-[75%] h-[100px] mt-2">{{ $bill_message }}</textarea>
