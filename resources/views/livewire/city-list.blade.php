    ...
    @if ($countries->count() > 0)
        <div class="flex gap-2">
            <x-select-input class="w-[200px] rounded-[32px]" id="region_id" label="" :values="$regions->pluck('name', 'id')"
                wire:model="selected_region_id" wire:change="update_parent_selected_region($event.target.value)"></x-select-input>
            <x-select-input class="w-[200px] rounded-[32px]" id="country_id" label="" :values="$countries->pluck('name', 'id')"
                wire:model="selected_country_id" wire:change="update_parent_selected_country($event.target.value)"></x-select-input>
        </div>
